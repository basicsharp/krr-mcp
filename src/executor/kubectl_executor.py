...
class KubectlExecutor:
    ...
    async def create_transaction(
        ...
        for change in changes:
            try:
                ...
                raise KubectlError(f"Failed to generate command for {change.object_name}: {str(e)}")
    ...
    async def execute_transaction(
        ...
        try:
            ...
            if transaction.execution_mode == ExecutionMode.SINGLE:
                ...
                raise KubectlError(f"Unsupported execution mode: {transaction.execution_mode}")
    ...
    async def _execute_single_mode(
        ...
        for i, command in enumerate(transaction.commands):
            ...
            if not result.is_successful() and not transaction.should_continue_on_failure():
                self.logger.warning(
    ...
    async def _execute_single_command(self, command: KubectlCommand) -> ExecutionResult:
        ...
        try:
            ...
            if result.exit_code == 0:
                ...
                affected_resources = self._parse_affected_resources(result.stdout, command)
    ...
    async def _execute_mock_command(self, command: KubectlCommand, result: ExecutionResult) -> ExecutionResult:
    ...
    def _parse_affected_resources(self, stdout: str, command: KubectlCommand) -> List[Dict[str, str]]:
    ...
    async def _create_rollback_snapshot(self, transaction: ExecutionTransaction) -> Optional[str]:
        ...
        try:
            ...
            for command in transaction.commands:
                ...
                if manifest:
                    ...
                    rollback_cmd = f"kubectl apply -f -"  # Would apply the original manifest
    ...
    async def _get_current_manifest(
        ...
        try:
            cmd_args = [
                "kubectl", "get", resource_type.lower(), resource_name,
                "--namespace", namespace,
                "--output", "json",
            ]
    ...
    def generate_execution_report(self, transaction: ExecutionTransaction) -> ExecutionReport:
        ...
        """
        successful_results = [r for r in transaction.command_results if r.is_successful()]
        failed_results = transaction.get_failed_commands()
        ...
        recommendations = []
        if failed_results:
            recommendations.append("Review failed commands and consider rollback if needed")
