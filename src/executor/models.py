...
class KubectlCommand(BaseModel):
    ...
    manifest_content: Optional[str] = Field(
        None, description="Manifest content if applicable"
    )
...
class ExecutionResult(BaseModel):
    ...
    affected_resources: List[Dict[str, str]] = Field(
        default_factory=list, description="Resources affected"
    )
...
class ExecutionTransaction(BaseModel):
    ...
    transaction_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Unique transaction ID"
    )
    ...
    completed_at: Optional[datetime] = Field(
        None, description="Transaction completion time"
    )

    # Commands and execution
    commands: List[KubectlCommand] = Field(
        ..., description="Commands in this transaction"
    )
    execution_mode: ExecutionMode = Field(
        ExecutionMode.SINGLE, description="Execution mode"
    )
    dry_run: bool = Field(False, description="Whether this is a dry-run transaction")

    # Results
    command_results: List[ExecutionResult] = Field(
        default_factory=list, description="Results of executed commands"
    )
    overall_status: ExecutionStatus = Field(
        ExecutionStatus.PENDING, description="Overall transaction status"
    )
    ...
    rollback_snapshot_id: Optional[str] = Field(
        None, description="Associated rollback snapshot ID"
    )
    ...
    def calculate_progress(self) -> Dict[str, Any]:
        ...
        completed = len(
            [r for r in self.command_results if r.status == ExecutionStatus.COMPLETED]
        )
        failed = len(
            [r for r in self.command_results if r.status == ExecutionStatus.FAILED]
        )
        in_progress = len(
            [r for r in self.command_results if r.status == ExecutionStatus.IN_PROGRESS]
        )
    ...
    def _estimate_remaining_time(self) -> Optional[float]:
        ...
        completed_results = [
            r for r in self.command_results if r.duration_seconds is not None
        ]
...
class ExecutionReport(BaseModel):
    ...
    # Resource impact
    resources_modified: List[Dict[str, str]] = Field(
        ..., description="Resources that were modified"
    )
    namespaces_affected: List[str] = Field(
        ..., description="Namespaces that were affected"
    )

    # Detailed results
    command_summaries: List[Dict[str, Any]] = Field(
        ..., description="Summary of each command"
    )
    error_summary: Optional[str] = Field(
        None, description="Summary of errors if any occurred"
    )

    # Recommendations
    recommendations: List[str] = Field(
        default_factory=list, description="Post-execution recommendations"
    )
    rollback_available: bool = Field(False, description="Whether rollback is available")
    rollback_snapshot_id: Optional[str] = Field(
        None, description="Rollback snapshot ID if available"
    )
...
class KubectlTimeoutError(KubectlError):
    ...
    def __init__(
        self, message: str, timeout_seconds: int, command: Optional[str] = None
    ):
...
class KubectlResourceNotFoundError(KubectlError):
    ...
    def __init__(
        self, message: str, resource_type: str, resource_name: str, namespace: str
    ):
