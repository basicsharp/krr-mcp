...
class SafetyValidator:
    ...
    def validate_changes(self, changes: List[ResourceChange]) -> SafetyAssessment:
        ...
        high_impact_changes = sum(
            1 for change in changes if self._is_high_impact_change(change)
        )
    ...
    def _check_resource_limits(
        ...
        for change in changes:
            ...
            if (
                ...
                and change.cpu_change_percent > self.config.MAX_CPU_INCREASE_PERCENT
            ):
                warnings.append(
                    SafetyWarning(
            ...
            if (
                ...
                and change.memory_change_percent
                > self.config.MAX_MEMORY_INCREASE_PERCENT
            ):
                warnings.append(
                    SafetyWarning(
                        level=RiskLevel.CRITICAL,
    ...
    def _check_production_namespaces(
        self, changes: List[ResourceChange]
    ) -> List[SafetyWarning]:
            ...
            if change.memory_change_percent:
                if change.memory_change_percent > 1000:  # >1000% increase
                    extreme_changes.append(
                        f"Memory: {change.memory_change_percent:.1f}%"
                    )
    ...
    def _check_simultaneous_changes(
        self, changes: List[ResourceChange]
    ) -> List[SafetyWarning]:
        ...
        for change in changes:
            if any(
                re.match(pattern, change.namespace.lower())
                for pattern in self.config.PRODUCTION_NAMESPACE_PATTERNS
            ):
    ...
    def _is_high_impact_change(self, change: ResourceChange) -> bool:
        ...
        if (
            change.cpu_change_percent
            and abs(change.cpu_change_percent)
            > self.config.HIGH_IMPACT_THRESHOLD_PERCENT
        ):
            return True
        if (
            change.memory_change_percent
            and abs(change.memory_change_percent)
            > self.config.HIGH_IMPACT_THRESHOLD_PERCENT
        ):
            return True
        return False
    ...
    def _count_critical_workloads(self, changes: List[ResourceChange]) -> int:
        ...
        for change in changes:
            workload_name = change.object_name.lower()
            if any(
                re.match(pattern, workload_name)
                for pattern in self.config.CRITICAL_WORKLOAD_PATTERNS
            ):
                critical_count += 1
    ...
    def _calculate_total_cpu_change(self, changes: List[ResourceChange]) -> float:
        ...
        cpu_changes = [
            c.cpu_change_percent for c in changes if c.cpu_change_percent is not None
        ]
    ...
    def _calculate_total_memory_change(self, changes: List[ResourceChange]) -> float:
        ...
        memory_changes = [
            ...
            for c in changes
            if c.memory_change_percent is not None
        ]
        return sum(memory_changes) / len(memory_changes) if memory_changes else 0.0

    def _requires_gradual_rollout(
        self, changes: List[ResourceChange], warnings: List[SafetyWarning]
    ) -> bool:
        ...
        if (
            high_impact_count
            >= self.config.GRADUAL_ROLLOUT_TRIGGERS["multiple_critical_workloads"]
        ):
            return True
        ...
        for change in changes:
            if (
                change.cpu_change_percent
                and abs(change.cpu_change_percent)
                > self.config.GRADUAL_ROLLOUT_TRIGGERS["large_resource_change"]
            ) or (
                change.memory_change_percent
                and abs(change.memory_change_percent)
                > self.config.GRADUAL_ROLLOUT_TRIGGERS["large_resource_change"]
    ...
    def _requires_monitoring(
        ...
        if any(
            w.level in [RiskLevel.MEDIUM, RiskLevel.HIGH, RiskLevel.CRITICAL]
            for w in warnings
        ):
            return True
