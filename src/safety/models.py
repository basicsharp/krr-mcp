...
class ResourceChange(BaseModel):
    ...
    object_kind: str = Field(..., description="Kubernetes object kind (e.g., Deployment)")
    ...
    # Calculated fields
    cpu_change_percent: Optional[float] = Field(None, description="CPU change percentage")
    memory_change_percent: Optional[float] = Field(None, description="Memory change percentage")
    
    estimated_cost_impact: Optional[float] = Field(None, description="Estimated monthly cost impact")
    ...
    def calculate_impact(self) -> None:
        ...
        if current_memory and proposed_memory and current_memory > 0:
            self.memory_change_percent = ((proposed_memory - current_memory) / current_memory) * 100
...
class SafetyAssessment(BaseModel):
    ...
    total_resources_affected: int = Field(..., description="Total number of resources affected")
    
    warnings: List[SafetyWarning] = Field(default_factory=list, description="List of safety warnings")
    
    # Impact summary
    total_cpu_change_percent: Optional[float] = Field(None, description="Total CPU change percentage")
    total_memory_change_percent: Optional[float] = Field(None, description="Total memory change percentage")
    estimated_monthly_cost_change: Optional[float] = Field(None, description="Estimated monthly cost change")
    
    # Safety metrics
    high_impact_changes: int = Field(0, description="Number of high-impact changes")
    critical_workloads_affected: int = Field(0, description="Number of critical workloads affected")
    production_namespaces_affected: List[str] = Field(default_factory=list, description="Production namespaces affected")
    
    # Recommendations
    requires_gradual_rollout: bool = Field(False, description="Whether gradual rollout is recommended")
    requires_monitoring: bool = Field(False, description="Whether enhanced monitoring is required")
    requires_backup: bool = Field(False, description="Whether backup is required before execution")


class ConfirmationToken(BaseModel):
    """Represents a confirmation token for user approval."""
    
    token_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique token ID")
    secret: str = Field(default_factory=lambda: secrets.token_urlsafe(32), description="Secret token value")
    
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Token creation time")
    expires_at: datetime = Field(..., description="Token expiration time")
    
    # Associated data
    changes: List[ResourceChange] = Field(..., description="Changes this token authorizes")
    safety_assessment: SafetyAssessment = Field(..., description="Safety assessment for these changes")
    ...
    def set_expires_at(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(values, dict) and 'expires_at' not in values:
            ...
            values['expires_at'] = created_at + timedelta(minutes=5)  # Default 5-minute expiration
        return values
...
class AuditLogEntry(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Entry timestamp")
    
    # Operation details
    operation: str = Field(..., description="Operation performed")
    status: str = Field(..., description="Operation status (requested, approved, executed, failed, rolled_back)")
    
    # User and context
    user_context: Dict[str, Any] = Field(default_factory=dict, description="User context information")
    cluster_context: Dict[str, str] = Field(default_factory=dict, description="Cluster context")
    
    # Changes and confirmations
    changes: List[ResourceChange] = Field(default_factory=list, description="Changes involved in this operation")
    confirmation_token_id: Optional[str] = Field(None, description="Associated confirmation token ID")
    safety_assessment: Optional[SafetyAssessment] = Field(None, description="Safety assessment")
    
    # Results
    execution_results: Dict[str, Any] = Field(default_factory=dict, description="Execution results")
    rollback_info: Optional[Dict[str, Any]] = Field(None, description="Rollback information if applicable")
...
class RollbackSnapshot(BaseModel):
    ...
    operation_id: str = Field(..., description="ID of the operation this snapshot is for")
    confirmation_token_id: str = Field(..., description="Associated confirmation token ID")
    
    # Snapshot data
    original_manifests: List[Dict[str, Any]] = Field(..., description="Original Kubernetes manifests")
    rollback_commands: List[str] = Field(..., description="Commands to execute rollback")
    
    # Metadata
    cluster_context: Dict[str, str] = Field(..., description="Cluster context at time of snapshot")
    affected_resources: List[Dict[str, str]] = Field(..., description="List of affected resources")
    ...
    def set_defaults(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(values, dict):
            ...
            if 'expires_at' not in values:
                created_at = values.get('created_at', datetime.now(timezone.utc))
                values['expires_at'] = created_at + timedelta(days=7)  # Default 7-day retention
