...
class ConfirmationManager:
    ...
    def validate_confirmation_token(self, token_id: str) -> Dict[str, Any]:
        ...
        if token.used:
            self.logger.warning("Token already used", token_id=token_id, used_at=token.used_at)
            return {
                "valid": False,
        ...
        if token.is_expired():
            self.logger.warning("Token expired", token_id=token_id, expires_at=token.expires_at)
            return {
                "valid": False,
    ...
    def cleanup_expired_tokens(self) -> int:
        ...
        """
        expired_tokens = [
            token_id for token_id, token in self._confirmation_tokens.items()
            if token.is_expired()
        ]
    ...
    def cleanup_expired_snapshots(self) -> int:
        ...
        """
        expired_snapshots = [
            snapshot_id for snapshot_id, snapshot in self._rollback_snapshots.items()
            if snapshot.is_expired()
        ]
        ...
        if expired_snapshots:
            self.logger.info("Cleaned up expired snapshots", count=len(expired_snapshots))
    ...
    def _generate_confirmation_prompt(
        ...
        # Add change details
        for i, change in enumerate(changes[:10], 1):  # Limit to first 10 for readability
            prompt_lines.extend([
            ...
            if change.memory_change_percent:
                prompt_lines.append(f"   Memory Change: {change.memory_change_percent:+.1f}%")
        ...
        if safety_assessment.warnings:
            ...
            for warning in safety_assessment.warnings[:5]:  # Limit to first 5 warnings
                prompt_lines.append(f"  - {warning.level.value.upper()}: {warning.message}")
            
            if len(safety_assessment.warnings) > 5:
                prompt_lines.append(f"  ... and {len(safety_assessment.warnings) - 5} more warnings")
    ...
    def _generate_changes_summary(self, changes: List[ResourceChange]) -> Dict[str, Any]:
