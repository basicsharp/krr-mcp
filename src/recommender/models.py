...
class KrrRecommendation(BaseModel):
    ...
    # Resource values
    current_requests: ResourceValue = Field(..., description="Current resource requests")
    current_limits: ResourceValue = Field(..., description="Current resource limits")
    recommended_requests: ResourceValue = Field(..., description="Recommended resource requests")
    recommended_limits: ResourceValue = Field(..., description="Recommended resource limits")
    
    # Recommendation metadata
    severity: RecommendationSeverity = Field(RecommendationSeverity.MEDIUM, description="Recommendation severity")
    potential_savings: Optional[float] = Field(None, description="Potential cost savings estimate")
    confidence_score: Optional[float] = Field(None, description="Confidence in recommendation (0.0-1.0)")
    
    # Analysis details
    analysis_period: Optional[str] = Field(None, description="Period analyzed (e.g., '7d')")
    cpu_usage_percentile: Optional[float] = Field(None, description="CPU usage percentile analyzed")
    memory_usage_percentile: Optional[float] = Field(None, description="Memory usage percentile analyzed")
    ...
    def calculate_impact(self) -> Dict[str, Any]:
        ...
        if self.current_requests.cpu and self.recommended_requests.cpu:
            ...
            if current_cpu and recommended_cpu:
                ...
                if current_cpu > 0:
                    impact["cpu_change_percent"] = ((recommended_cpu - current_cpu) / current_cpu) * 100
        ...
        if self.current_requests.memory and self.recommended_requests.memory:
            ...
            recommended_memory = self._parse_memory_value(self.recommended_requests.memory)
            
            if current_memory and recommended_memory:
                impact["memory_change"] = recommended_memory - current_memory
                if current_memory > 0:
                    impact["memory_change_percent"] = ((recommended_memory - current_memory) / current_memory) * 100
...
class RecommendationFilter(BaseModel):
    ...
    object_name_pattern: Optional[str] = Field(None, description="Filter by object name pattern")
    resource_type: Optional[ResourceType] = Field(None, description="Filter by resource type")
    severity: Optional[RecommendationSeverity] = Field(None, description="Filter by severity")
    min_potential_savings: Optional[float] = Field(None, description="Minimum potential savings")
...
class KrrScanResult(BaseModel):
    ...
    # Scan parameters
    namespaces_scanned: List[str] = Field(..., description="Namespaces included in scan")
    analysis_period: str = Field(..., description="Historical period analyzed")
    
    # Results
    recommendations: List[KrrRecommendation] = Field(..., description="Generated recommendations")
    total_recommendations: int = Field(..., description="Total number of recommendations")
    
    # Summary statistics
    potential_total_savings: Optional[float] = Field(None, description="Total potential savings")
    recommendations_by_severity: Dict[str, int] = Field(default_factory=dict, description="Count by severity")
    
    # Execution metadata
    scan_duration_seconds: Optional[float] = Field(None, description="Time taken for scan")
    ...
    def filter_recommendations(self, filter_criteria: RecommendationFilter) -> List[KrrRecommendation]:
        """Filter recommendations based on criteria."""
        filtered = self.recommendations
        
        if filter_criteria.namespace:
            filtered = [r for r in filtered if r.object.namespace == filter_criteria.namespace]
        
        if filter_criteria.object_kind:
            filtered = [r for r in filtered if r.object.kind == filter_criteria.object_kind]
        ...
        if filter_criteria.min_potential_savings:
            filtered = [r for r in filtered 
                       if r.potential_savings and r.potential_savings >= filter_criteria.min_potential_savings]
        
        if filter_criteria.min_confidence_score:
            filtered = [r for r in filtered 
                       if r.confidence_score and r.confidence_score >= filter_criteria.min_confidence_score]
...
class KrrError(Exception):
    ...
    def __init__(self, message: str, error_code: str = "KRR_ERROR", details: Optional[Dict[str, Any]] = None):
