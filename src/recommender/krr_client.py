...
class KrrClient:
    ...
    async def scan_recommendations(
        ...
        if self.mock_responses:
            return await self._generate_mock_scan_result(namespace, strategy, history_duration)
        ...
        try:
            ...
            if isinstance(e, KrrError):
                raise
            raise KrrExecutionError(f"Unexpected error during krr scan: {str(e)}", exit_code=-1)
    ...
    async def _execute_krr_command(self, cmd_args: List[str], timeout: int = 300) -> str:
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                if "prometheus" in stderr_str.lower() or "connection refused" in stderr_str.lower():
                    raise PrometheusConnectionError(
                        f"Failed to connect to Prometheus: {stderr_str}",
                        prometheus_url=self.prometheus_url,
                    )
                
                if "context" in stderr_str.lower() or "kubeconfig" in stderr_str.lower():
            ...
            if isinstance(e, (KrrExecutionError, PrometheusConnectionError, KubernetesContextError)):
                raise
            raise KrrExecutionError(f"Unexpected error executing krr: {str(e)}", exit_code=-1)
    ...
    def _parse_single_recommendation(self, raw_rec: Dict[str, Any]) -> KrrRecommendation:
    ...
    def _cache_scan_result(self, cache_key: str, scan_result: KrrScanResult) -> None:
        ...
        self.logger.debug("Cached scan result", cache_key=cache_key, ttl=self.cache_ttl_seconds)
    ...
    async def _generate_mock_scan_result(
        ...
        if namespace:
            mock_recommendations = [
                ...
                if r.object.namespace == namespace
            ]
