...
class TestFullRecommendationWorkflow:
    ...
    async def test_complete_workflow_success(self, test_server, mock_krr_response):
        ...
        confirmation_result = await test_server.request_confirmation(
            changes=preview_result["impact_analysis"],
            risk_level="medium"
        )
    ...
    @pytest.mark.asyncio
    async def test_workflow_with_safety_rejection(self, test_server, dangerous_recommendations):
...
class TestErrorRecoveryWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_kubernetes_api_failure_recovery(self, test_server, mock_krr_response):
        ...
        with patch('src.executor.kubectl_executor.KubectlExecutor.execute_command') as mock_execute:
            ...
            assert "kubernetes" in apply_result["error"].lower() or "api" in apply_result["error"].lower()
...
class TestAuditTrailWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_complete_audit_trail(self, test_server, mock_krr_response, caplog_structured):
        ...
        expected_fields = [
            "timestamp", "operation", "user_context", "changes", 
            "confirmation_token", "result", "execution_id"
        ]
...
class TestSafetyWorkflows:
    ...
    async def test_extreme_change_detection(self, test_server):
        ...
        assert any("extreme" in warning.lower() for warning in safety_assessment["warnings"])
